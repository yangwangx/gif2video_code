...
class Hollywood2_uvm_train(DD.Dataset):
    def __init__(self, subset='train',
                 HW=[150, 200], scale=[150, 128, 112], HW_=[128, 128], T_=16):
        super(Hollywood2_uvm_train, self).__init__()
    ...
    def __getitem__(self, index):
        ...
        for i, t in enumerate(frmInx):
            fx = Image.open('%s/x_%06d.jpg'%(frameDir, min(t, maxFrm)))
            fy = Image.open('%s/y_%06d.jpg'%(frameDir, min(t, maxFrm)))
            # scale, crop, flip, rescale
            fx = fx.resize((W, H), Image.BILINEAR)
            fy = fy.resize((W, H), Image.BILINEAR)
            fx = fx.crop((r_crop[2], r_crop[0], r_crop[3]+1, r_crop[1]+1)) # left-inclusive, [a, b)
            fy = fy.crop((r_crop[2], r_crop[0], r_crop[3]+1, r_crop[1]+1)) # left-inclusive, [a, b)
            if r_flip:
                fx = fx.transpose(Image.FLIP_LEFT_RIGHT)
                fy = fy.transpose(Image.FLIP_LEFT_RIGHT)
            fx = fx.resize((W_, H_), Image.BILINEAR)
            fy = fy.resize((W_, H_), Image.BILINEAR)
            fx = self.toTensor(fx)*255-128
            fy = self.toTensor(fy)*255-128
            if r_flip:
                fx = -fx
            fm = torch.cat([fx, fy, torch.pow(torch.pow(fx, 2)+torch.pow(fy, 2), 0.5)], 0)
            clip[:,i,:,:].copy_(fm)
...
class Hollywood2_uvm_feature(DD.Dataset):
    ...
    def __getitem__(self, index):
        ...
        for i, t in enumerate(range(1, maxFrm+1)):
            fx = Image.open('%s/x_%06d.jpg'%(frameDir, t))
            fy = Image.open('%s/y_%06d.jpg'%(frameDir, t))
            fx = fx.resize((W, H), Image.BILINEAR)
            fy = fy.resize((W, H), Image.BILINEAR)
            fx = self.toTensor(fx)*255-128
            fy = self.toTensor(fy)*255-128
...
if __name__ == '__main__':
    dataset = Hollywood2_uvm_train(subset='train')
    clip, label, cls = random.choice(dataset)
    TT.ToPILImage()(clip[:, 8, :, :]/255).show()
    dataset = Hollywood2_uvm_feature(subset='train')
    clip, label, cls = random.choice(dataset)
