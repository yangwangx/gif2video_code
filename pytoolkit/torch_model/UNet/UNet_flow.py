...
def double_conv(in_ch, out_ch, kernel_size=3, use_bn=False, leak=0.1):
...
class upconv(nn.Module):
    def __init__(self, down_ch, up_ch, out_ch, bilinear=True):
...
class UNet_flow(nn.Module):
    def __init__(self, in_ch, out_ch, ch=32):
        super(UNet_flow, self).__init__()
        self.inconv    =   inconv(in_ch,  ch*1, kernel_size=7)
        self.downconv1 = downconv( ch*1,  ch*2, kernel_size=5)
        self.downconv2 = downconv( ch*2,  ch*4)
        self.downconv3 = downconv( ch*4,  ch*8)
        self.downconv4 = downconv( ch*8, ch*16)
        self.downconv5 = downconv(ch*16, ch*16)

        self.upconv1 = upconv(ch*16, ch*16, ch*16)
        self.upconv2 = upconv(ch*16,  ch*8,  ch*8)
        self.upconv3 = upconv( ch*8,  ch*4,  ch*4)
        self.upconv4 = upconv( ch*4,  ch*2,  ch*2)
        self.upconv5 = upconv( ch*2,  ch*1,  ch*1)
...
if __name__ == '__main__':
    model = UNet_flow(in_ch=3, out_ch=3, ch=32)
